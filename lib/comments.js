 ...
 */
const createComments = async (report, token) => {
    if (!report || !token) {
    ...
    if (concurrentRequestsLimit >= comments.length) {
        comments.forEach((data) => {
            core.debug(`POST /repos/${owner}/${repo}/issues/${issue}/comments with ${JSON.stringify(data)}`);
            promises.push(octokit.request(`POST /repos/${owner}/${repo}/issues/${issue}/comments`, data));
        });
        await Promise.allSettled(promises).catch((issue) => {
            core.error(issue);
        });
