...
describe('pmd-github-action-validator', function() {
    ...
    it('validate input version should throw', () => {
        expect(() => validator.validateVersion('; /bin/bash')).toThrow('Invalid version');
    });
    ...
    it('validate source path should throw', () => {
        expect(() => validator.validateSourcePath('/src')).toThrow('Invalid sourcePath');
        expect(() => validator.validateSourcePath('; /bin/bash')).toThrow('Invalid sourcePath');
    });

    it('validate rulesets list', () => {
        expect(validator.validateRulesets('ruleset.xml')).toBe('ruleset.xml');
        expect(validator.validateRulesets('rulesets/java/quickstart.xml')).toBe('rulesets/java/quickstart.xml');
        expect(validator.validateRulesets('rulesets/java/quickstart.xml,ruleset.xml')).toBe('rulesets/java/quickstart.xml,ruleset.xml');
        expect(validator.validateRulesets('rulesets/java/quickstart.xml, ruleset.xml')).toBe('rulesets/java/quickstart.xml,ruleset.xml');
    });
